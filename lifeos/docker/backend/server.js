...
function createToken(userId, email) {
  ...
  const signature = crypto
    .createHmac(
      "sha256",
      process.env.JWT_SECRET || "lifeos-self-hosted-secret-change-me",
    )
    .update(data)
    .digest("base64url");
  ...
  try {
    if (!dbClient) {
      ...
      try {
        ...
        sendJson(res, 200, {
          isSetup: false,
          needsSetup: true,
          isDocker: !!process.env.DB_HOST,
        });
        return;
      }
    }
    const rows = await query(
      "SELECT setup_complete, db_type FROM app_settings LIMIT 1",
    );
    ...
    if (rows.length > 0 && rows[0].setup_complete) {
      ...
      sendJson(res, 200, {
        ...
        hasAdmin,
        isDocker,
        dbType: rows.length > 0 ? rows[0].db_type : dbType,
      });
    }
  } catch {
    sendJson(res, 200, {
...
routes["POST /api/setup/admin"] = async (req, res) => {
  ...
  try {
    ...
    if (dbType === "postgresql") {
      ...
      const adminRows = await query("SELECT id FROM users WHERE email = $1", [
        email,
      ]);
      ...
      const adminRows = await query("SELECT id FROM users WHERE email = ?", [
        email,
      ]);
      const realAdminId = adminRows[0].id;
      await query(
...
routes["POST /api/auth/register"] = async (req, res) => {
  ...
  try {
    ...
    await query(
      "INSERT INTO user_roles (id, user_id, role) VALUES ($1, $2, $3::app_role)",
      [uuid(), userId, "user"],
    );
    await query(
      "INSERT INTO profiles (id, user_id, full_name, email) VALUES ($1, $2, $3, $4)",
...
const LICENSE_VERIFY_URL =
  ...
  "https://abcytwvuntyicdknpzju.supabase.co/functions/v1/verify-license";
const LICENSE_ENCRYPTION_KEY =
  process.env.LICENSE_ENC_KEY || "ITSupportBD_SecureKey_2024";
...
async function getOrCreateInstallationId() {
  try {
    if (dbType === "postgresql") {
      const rows = await query(
        "SELECT setting_value FROM license_settings WHERE setting_key = 'installation_id'",
      );
...
async function getLicenseSetting(key) {
  try {
    const rows = await query(
      "SELECT setting_value FROM license_settings WHERE setting_key = $1",
      [key],
    );
...
async function verifyLicenseWithPortal(licenseKey, force = false) {
  ...
  if (
    !force &&
    licenseCache.last_verified &&
    Date.now() / 1000 - licenseCache.last_verified <
  ...
  try {
    ...
    await setLicenseSetting(
      "last_successful_portal_connection",
      String(licenseCache.last_successful_connection),
    );
    ...
    console.log(
      `LICENSE_INFO: Verification completed. Status: ${licenseCache.status}`,
    );
    return licenseCache;
  } catch (err) {
    ...
    if (!lastConn) {
      const stored = await getLicenseSetting(
        "last_successful_portal_connection",
      );
...
routes["POST /api/license/verify"] = async (req, res) => {
  ...
  try {
    ...
    sendJson(res, 500, {
      success: false,
      message: "License verification failed: " + err.message,
    });
  }
};
...
async function handlePostgrestRequest(req, res, tableName) {
    ...
    try {
      ...
      if (returnMinimal) {
        ...
        sendRestJson(
          res,
          201,
          results.length === 1 && !Array.isArray(body) ? results[0] : results,
        );
      }
    } catch (err) {
      console.error(`PostgREST POST ${tableName} error:`, err.message);
      sendRestJson(res, 500, {
        message: err.message,
        details: err.detail || "",
      });
    }
  ...
  if (req.method === "PATCH") {
    try {
      const body = await parseBody(req);
      const { where, values, nextIdx } = parsePostgrestFilters(
        queryParams,
        tableName,
      );
  ...
  try {
    ...
    if (urlPath.startsWith("/auth/v1/")) {
      ...
      if (urlPath === "/auth/v1/token" && req.method === "POST") {
        ...
        sendRestJson(res, 400, {
          error: "self_hosted_mode",
          error_description:
            "Use /api/auth/login for self-hosted authentication",
        });
        return;
      }
    ...
    if (urlPath.startsWith("/functions/v1/")) {
      ...
      const functionResponses = {
        "send-task-reminders": {
          success: true,
          message: "Task reminders processed (local mode)",
          sent: 0,
        },
        "send-habit-reminders": {
          success: true,
          message: "Habit reminders processed (local mode)",
          sent: 0,
        },
        "send-family-event-reminders": {
          success: true,
          message: "Event reminders processed (local mode)",
          sent: 0,
        },
        "send-loan-reminders": {
          success: true,
          message: "Loan reminders processed (local mode)",
          sent: 0,
        },
        "send-push-notification": {
          success: true,
          message: "Push notification skipped (local mode)",
        },
        "send-email-notification": {
          success: true,
          message: "Email notification skipped (local mode)",
        },
        "send-email-otp": {
          success: true,
          message: "Email OTP skipped (local mode)",
        },
        "send-smtp-email": {
          success: true,
          message: "SMTP email skipped (local mode)",
        },
        "send-task-assignment-notification": {
          success: true,
          message: "Assignment notification skipped (local mode)",
        },
        "google-calendar-sync": {
          success: true,
          message: "Google Calendar sync not available in local mode",
          events: [],
        },
        "microsoft-calendar-sync": {
          success: true,
          message: "Microsoft Calendar sync not available in local mode",
          events: [],
        },
        "save-calendar-credentials": {
          success: true,
          message: "Calendar credentials not supported in local mode",
        },
        "manage-resend-key": {
          success: true,
          message: "Resend API not available in local mode",
        },
        "ai-assist": {
          success: true,
          content: "AI features require cloud mode or a configured API key.",
          provider: "local",
          error: "AI not available in self-hosted mode",
        },
      };

      const response = functionResponses[funcName] || {
        success: true,
        message: `Edge function '${funcName}' not available in self-hosted mode`,
      };
...
// Tables that should NOT be scoped by user_id in data API (SELECT/INSERT/UPSERT/UPDATE)
// These are shared/office tables visible to all authenticated users.
...
// Tables in DATA_NO_USER_SCOPE that still have a user_id column.
// DELETE operations on these should be scoped by user_id to avoid wiping
// another user's data (e.g. during a full restore in Docker multi-user mode).
const SHARED_TABLES_WITH_USER_ID = new Set([
  "support_units",
  "support_departments",
  "support_users",
  "device_categories",
...
// Tables that are truly global (no user_id column at all).
// DELETE on these wipes the whole table — intentional for full restore.
const GLOBAL_NO_USER_ID_TABLES = new Set([
  "device_transfer_history",
  "form_field_config",
  "module_config",
  "support_tickets",
...
async function handleDataSelect(req, res, tableName) {
  ...
  try {
    ...
    if (DATA_NO_USER_SCOPE.has(tableName)) {
      ...
      rows = await query(`SELECT * FROM ${tableName} WHERE user_id = $1`, [
        user.sub,
      ]);
    }
...
async function handleDataDelete(req, res, tableName) {
  ...
  try {
    if (GLOBAL_NO_USER_ID_TABLES.has(tableName)) {
      // Truly global tables have no user_id — delete all rows.
      await query(`DELETE FROM ${tableName}`);
    } else if (
      SHARED_TABLES_WITH_USER_ID.has(tableName) ||
      !DATA_NO_USER_SCOPE.has(tableName)
    ) {
      // Shared office tables (device_inventory, support_users, etc.) AND
      // regular user-scoped tables both have a user_id — scope the delete.
      await query(`DELETE FROM ${tableName} WHERE user_id = $1`, [user.sub]);
    } else {
      // Fallback for any remaining DATA_NO_USER_SCOPE tables not in the sets above
      // (e.g. app_settings, user_roles): delete all rows.
      await query(`DELETE FROM ${tableName}`);
    }
    sendJson(res, 200, { success: true });
  } catch (err) {
...
async function seedDefaultAdmin() {
  try {
    ...
    if (wizardCreated === "true") {
      console.log(
        "✅ Admin already configured via setup wizard — skipping default seed.",
      );
      return;
    }
    ...
    if (connected) {
      ...
      try {
        ...
        if (hasEnvKey || hasStoredKey) {
          ...
          console.log(
            `🔑 License status: ${result.status} - ${result.message}`,
          );
          ...
          setInterval(async () => {
            try {
              ...
              if (currentKey) {
                ...
                const recheckResult = await verifyLicenseWithPortal(
                  currentKey,
                  true,
                );
                console.log(
                  `🔄 License recheck: ${recheckResult.status} - ${recheckResult.message}`,
                );
              }
            } catch (recheckErr) {
              console.error(
                "⚠️ Periodic license recheck failed:",
                recheckErr.message,
              );
            }
          }, RECHECK_INTERVAL_MS);
      ...
      console.log(
        "✅ Backend initialization complete — ready to serve requests",
      );
    } else {
